...
public class Booktype_alter {
	...
	protected void createContents() {
		...
		shell.setBackgroundImage(SWTResourceManager.getImage(Booktype_alter.class, "/images/back.jpg"));
		shell.setBackgroundMode(SWT.INHERIT_DEFAULT);
		shell.setBackground(SWTResourceManager.getColor(SWT.COLOR_WIDGET_HIGHLIGHT_SHADOW));
		shell.setSize(1061, 735);
		shell.setText("\u4E66\u7C7B\u4FEE\u6539");
		...
		text.setBounds(438, 215, 165, 26);

		text_1 = new Text(shell, SWT.NONE);
		text_1.setBounds(438, 297, 165, 26);

		text_2 = new Text(shell, SWT.MULTI);
		text_2.setBounds(438, 374, 165, 56);
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (text.getText().trim().length() != 0 && text_1.getText().trim().length() != 0
					...
					if (t.altersort(sort)) {
						...
						messageBox.setMessage("修改书类成功");
						messageBox.open();
					} else {
						MessageBox messageBox = new MessageBox(shell, SWT.OK | SWT.ICON_ERROR);
						messageBox.setMessage("修改书类失败");
						messageBox.open();
					}
				} else {
					MessageBox messageBox = new MessageBox(shell, SWT.OK | SWT.ICON_ERROR);
		...
		btnNewButton.setBounds(438, 473, 165, 42);

		Label lblNewLabel_1 = new Label(shell, SWT.NONE);
		lblNewLabel_1.setImage(SWTResourceManager.getImage(Booktype_alter.class, "/images/MC.ico"));
		lblNewLabel_1.setBounds(200, 23, 138, 128);
	}
